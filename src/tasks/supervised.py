...
class SupervisedForecastTask(pl.LightningModule):
    def __init__(
        ...
        **kwargs
        ...
        """Constructor of SupervisedForecaseTask class

        Args:
            model (nn.Module): TGCN model
            regressor (str, optional): linear regressor. Defaults to "linear".
            loss (str, optional): loss function. Defaults to "mse".
            pre_len (int, optional): pre length. Defaults to 3.
            learning_rate (float, optional): learning rate of optimizer. Defaults to 1e-3.
            weight_decay (float, optional): weight decays. Defaults to 1.5e-3.
            feat_max_val (float, optional): feature max values. Defaults to 1.0.
        """
    ...
    def forward(self, x):
        """Generates the forward pass to the neural networks

        Args:
            x ([type]): input features
    ...
    def shared_step(self, batch, batch_idx):
        """Generates shared steps taking in the feature, labels and return the predictions and ground-truths

        Args:
            batch ([type]): batch size of input features and ground truth
            batch_idx ([type]): batch index

        Returns:
            [type]: predicted labels and ground-truths
        """
        x, y = batch
        num_nodes = x.size(2)
        predictions = self(x)
    ...
    def loss(self, inputs, targets):
        """Calculates either the MSE or MSE with regularizer loss between predicted value and ground-truth targets

        Args:
            inputs ([type]): input features
            targets ([type]): ground-truth targets

        Raises:
            NameError: if there is no specified loss function
    ...
    def training_step(self, batch, batch_idx):
        """Generates training step using only mse or mse with regularizer.
        ...
        Returns:
            [type]: predictions and ground-truth labels
        """
    ...
    def test_step(self, batch, batch_idx):
        """Generates testing step - nothing since there is not testing set, just 80% training and 20% testing
