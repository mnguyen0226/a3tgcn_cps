...
def plot_result_valve(test_result, test_label, path, hour = 24):
    """Plots test datasets ground truth labels vs predictions on ground truth features
...
def plot_result_junction(test_result, test_label, path, hour = 24):
    """Plots test datasets ground truth labels vs predictions on ground truth features
    ...
    fig1, axs = plt.subplots(6,2)
    hour_cal = float(hour) / 24.0
    fig_name = "Inlet & Outlet Pressure of Junctions # 1-12 in " + str(hour_cal) + " Day(s)"
    fig1.suptitle(fig_name)
