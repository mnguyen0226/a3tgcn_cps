...
class GCN(object):
    ...
    def init_state(self, batch_size):
        state = tf.zeros(
            shape=[batch_size, self._num_nodes * self._num_units], dtype=tf.float32
        )
        return state
    ...
    def _gconv(self, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            x1 = tf.reshape(
                x1, shape=[self._num_nodes, self._batch_size, self._input_dim]
            )
            x1 = tf.reshape(
                x1, shape=[self._num_nodes * self._batch_size, self._input_dim]
            )

            weights = weight_variable_glorot(
                self._input_dim, self.output_size, name="weights"
            )
            self.hidden1 = self._activation(tf.matmul(x1, weights))

            # output
            weights = weight_variable_glorot(
                self.output_size, self._output_dim, name="weights"
            )
            self.output = tf.matmul(self.hidden1, weights)
            self.output = tf.reshape(
                self.output, shape=[self._num_nodes, self._batch_size, self._output_dim]
            )
